...
export const QrCode: React.FC<QrCodeProps> = ({ clientId }) => {
  ...
  const handleQrCode = async (clientId: string) => {
    try {
      const response = await getClientConfig(clientId, true);
      console.log("qrcode",response.data)
      setQrCodeData(response.data); // Extract the data from the response object
    } catch (error) {
    ...
    <div>
        ...
        <div className="modal ">
          <div className="modal-box relative w-full bg-gray-900 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-80 border border-gray-100">
            ...
            <p className="py-4 text-sm text-white font-medium">You will get the client's config qrcode here..</p>
